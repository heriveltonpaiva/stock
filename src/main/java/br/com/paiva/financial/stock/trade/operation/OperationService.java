...
public class OperationService {
  ...
  public OperationResponse findByFilter(final OperationSearchRequest searchRequest) {
    final List<Operation> operations = operationRepository.findByFilter(searchRequest);
    TotalOperationFiltered totalOperation = new TotalOperationFiltered();
    operations.forEach(
        operation -> {
          totalOperationService.calculateTotalOperationFiltered(totalOperation, operation);
        });
  ...
  private void calculateOperationValues(final Operation op, final BrokerType brokerType) {
    if (op.getType() == OperationType.SELL) {
      ...
      StockPosition lastStockPosition =
          stockPositions.stream().filter(s -> s.getBroker().equals(brokerType)).findFirst().get();
      log.info("Stock position was found stockPosition={}", lastStockPosition);
      log.info(
          "{} Stock position purchasePrice={}",
          op.getStockName(),
          lastStockPosition.getTotalPurchased());

      op.setPurchasePrice(
          StockUtils.getToY(
              lastStockPosition.getQuantity(),
              lastStockPosition.getTotalPurchased(),
              op.getQuantity()));
  ...
  private TotalOperation updateTotalOperation(Operation op) {
    ...
    return totalOperationService.createOrUpdate(noteDate, op);
  }
